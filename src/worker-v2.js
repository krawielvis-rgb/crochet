...
async function publishStructured(r,e){
  ...
  try{
    ...
    existing=list.find(p=>p.slug===s)||null;
  }catch{}
  ...
  const post={
    ...
    title,
    description,
    category:String(d.category||'Crochet tutorial').trim()||'Crochet tutorial',
    readTime:String(d.readTime||'12 min read').trim()||'12 min read',
    image:imagePath,
    published:true,
    materials:String(d.materials||'').trim(),
    intro:String(d.intro||'').trim(),
    steps:String(d.steps||'').trim().split('\n').map(x=>x.trim()).filter(Boolean),
    checkpoint:String(d.checkpoint||'').trim(),
    troubleshooting:String(d.troubleshooting||'').trim(),
    finishing:String(d.finishing||'').trim(),
    variations:String(d.variations||'').trim(),
    care:String(d.care||'').trim(),
    rawContent:String(d.rawContent||'').trim()
  };

  // If structured fields are mostly empty but rawContent exists, use it as intro body
  if(!post.materials && !post.intro && !post.steps.length && post.rawContent){
    post.intro=post.rawContent;
  }
  ...
  const treeItems=[
    {path:`public/posts/${s}.html`,mode:'100644',type:'blob',sha:(await g(`${API}/git/blobs`,e,{method:'POST',body:JSON.stringify({content:html,encoding:'utf-8'})})).sha}
  ];
  if(imageBlob){
    treeItems.push({path:`public/images/pins/pin-${s}.${ext}`,mode:'100644',type:'blob',sha:(await g(`${API}/git/blobs`,e,{method:'POST',body:JSON.stringify({content:imageBlob,encoding:'base64'})})).sha});
  }

  let ps=await posts(e);
  ps=ps.filter(p=>p.slug!==s);
  // Store a lighter catalog entry + keep rich fields for dynamic rendering
  ps.unshift({
    slug:post.slug,
    title:post.title,
