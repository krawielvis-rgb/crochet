...
async function posts(e){let r=await fetch(`${RAW}/data/posts.json?x=${Date.now()}`);let live=r.ok?await r.json():[];if(!Array.isArray(live))live=[];let remote=live;if(live.length<10){const fb=await fetch('https://raw.githubusercontent.com/krawielvis-rgb/crochet/03cc176d8d0ef2bc06e02e1b192001b9cc22bfbf/data/posts.json');if(fb.ok){const base=await fb.json();const liveSlugs=new Set(live.map(p=>p&&p.slug));remote=[...live,...base.filter(p=>p&&!liveSlugs.has(p.slug))]}}const map=new Map((freshPosts||[]).map(p=>[p.slug,{...p}]));for(const p of remote){if(!p||!p.slug)continue;if(map.has(p.slug)){const base=map.get(p.slug);map.set(p.slug,{...base,...p,image:p.image||base.image})}else map.set(p.slug,p)}const ordered=[];const seen=new Set();for(const p of live){if(p&&p.slug&&map.has(p.slug)&&!seen.has(p.slug)){ordered.push(map.get(p.slug));seen.add(p.slug)}}for(const p of map.values()){if(p&&p.slug&&!seen.has(p.slug)){ordered.push(p);seen.add(p.slug)}}return ordered.filter(p=>{if(!p||p.published===false)return false;const s=String(p.slug||'').toLowerCase();if(s.includes('sunglass'))return false;if(s==='bat-wing-cardigan-crochet-bat-cardigan')return false;return true;}).map(p=>({...p,image:resolveImage(p)}))}
